...
const AUTOFILL_RETRY_DELAYS = [0, 100, 300, 700, 1500];
...
function perfLog(...args) {
  if (DEBUG_PERFORMANCE) {
    console.log('[AutoLogin:content]', ...args);
  }
}
...
async function autoFillIfEnabled() {
  ...
  if (result.ok && result.matchedFields === result.totalFields) {
    autofillCompleted = true;
    disconnectObserver();
    perfLog('fill success', `${Date.now() - autofillStartTime}ms`, result.message);
  } else {
    perfLog('fill attempt incomplete', result.message);
  }

  browserApi.runtime.sendMessage({ type: 'AUTO_FILL_RESULT', result, hostname });
  return { attempted: true, success: Boolean(result.ok && result.matchedFields === result.totalFields), result };
}
...
async function getCachedRule(hostname) {
  ...
  cachedRulePromise = browserApi.runtime
    .sendMessage({ type: 'GET_SITE_RULE', hostname })
    .then((response) => {
      cachedRule = response?.ok ? response.rule || null : null;
      perfLog('rule fetched', hostname);
      return cachedRule;
    })
    .catch(() => {
      cachedRule = null;
...
function setupMutationObserver() {
  if (mutationObserver || autofillCompleted) {
    return;
  }

  mutationObserver = new MutationObserver(() => {
    if (observerTimer || autofillCompleted) {
      return;
    }
...
function scheduleAutoFillAttempts() {
  ...
  clearRetryTimers();
  if (!autofillStartTime) {
    autofillStartTime = Date.now();
    perfLog('content script loaded');
  }
